...
const BusTrackerPage = () => {
  const [busLocation, setBusLocation] = useState({ lat: 0, long: 0 });
  ...
  const fetchBusRoute = async () => {
      try {
          ...
          setRoute(data.route); // Assume the route is an array of lat/long
      } catch (error) {
  ...
  return (
      <Container className='relative h-screen p-0 bg-gray-50' sx={{ height: '100vh', width: '100%', p: 0 }}>
          <div className='relative h-full'>
              <GoogleMap
                  onLoad={(map) => {
                      mapRef.current = map;
                      if (busLocation.lat !== 0 && busLocation.long !== 0) {
